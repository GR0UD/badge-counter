...
function generateBadge(options) {
  ...
  const rightInnerW = (hasIcon ? ICON_SIZE + GAP : 0) + countTextWidth;
  const rightSectionW = rightInnerW + PADDING * 2;
  const totalWidth = labelSectionW + rightSectionW;

  const midY = HEIGHT / 2;
  const textY = midY + FONT_SIZE * 0.38;

  const rightCenterX = labelSectionW + rightSectionW / 2;
  const groupW = rightInnerW;
  const groupStartX = rightCenterX - groupW / 2;

  const iconX = groupStartX;
  const countX =
    groupStartX + (hasIcon ? ICON_SIZE + GAP : 0) + countTextWidth / 2;
